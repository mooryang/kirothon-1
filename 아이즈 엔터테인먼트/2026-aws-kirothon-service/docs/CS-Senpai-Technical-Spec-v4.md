...
│   │   └── night-summarizer.md   # 야간 요약 + 오케스트레이터 에이전트
│   ├── powers/                   # [실행 레이어] 외부 서비스 연동 Power
│   │   └── night-ops/            # 야간 GitHub 모니터링 + 자동화 Power
│   │       ├── POWER.md          # 키워드 트리거 + Power 정의
│   │       ├── mcp.json          # GitHub MCP 서버 설정
│   │       └── steering/
│   │           ├── review-standards.md  # Power 전용 리뷰 기준
│   │           └── output-format.md     # Power 전용 출력 포맷
│   ├── skills/                   # [실행 레이어] 에이전트 on-demand 스킬
│   │   ├── ci-pattern-analysis/
│   │   │   └── SKILL.md          # CI 실패 패턴 분석 전문 지식
│   │   └── senpai-voice/
│   │       └── SKILL.md          # 전산과 선배 말투 생성 스킬
│   └── hooks/                    # [제어 레이어] 이벤트 트리거 자동화
│       ├── post-analysis.md      # 분석 완료 후 자동 분류 + DB 저장
│       ├── inject-repo-context.md # 분석 시작 시 레포 컨텍스트 자동 주입
│       └── save-result.md        # 에이전트 종료 시 결과 자동 저장
├── public/
│   └── fonts/
...
│  └────────────────────────────────────────────────────┘  │
│                         ▼                               │
│  ┌─── 실행 레이어 (Subagents + Powers + Skills) ─────┐  │
│  │  Custom Subagents (.kiro/agents/)                  │  │
│  │  ├── pr-reviewer.md      (PR 리뷰)                │  │
│  │  ├── ci-analyzer.md      (CI 분석)                 │  │
│  │  ├── issue-triager.md    (이슈 분류)               │  │
│  │  ├── security-scanner.md (보안 스캔)               │  │
│  │  └── night-summarizer.md (오케스트레이터 + 요약)   │  │
│  │                                                    │  │
│  │  night-summarizer가 #agents로 하위 에이전트 위임:  │  │
│  │  PR목록 → #pr-reviewer 병렬 위임 ─┐               │  │
│  │  CI실패 → #ci-analyzer 위임 ──────┤ Kiro 내부     │  │
│  │  이슈   → #issue-triager 위임 ────┘ 오케스트레이션 │  │
│  │  → 전부 완료 후 daily_summary 생성                 │  │
│  │                                                    │  │
│  │  Custom Power (.kiro/powers/night-ops/)            │  │
│  │  ├── POWER.md   (키워드 트리거: PR, CI, issue...)  │  │
│  │  ├── mcp.json   (GitHub MCP 8개 툴 연결)           │  │
│  │  └── steering/  (Power 전용 규칙)                  │  │
│  │  → 에이전트가 PR/CI/이슈 키워드 사용 시에만 로드   │  │
│  │  → 컨텍스트 절약 + 필요할 때만 GitHub 조작 가능    │  │
│  │                                                    │  │
│  │  Custom Skills (.kiro/skills/)                     │  │
│  │  ├── ci-pattern-analysis/ (CI 실패 패턴 전문 지식) │  │
│  │  └── senpai-voice/        (전산과 선배 말투 생성)   │  │
│  │  → 에이전트가 필요 시 on-demand 로드               │  │
│  │  → night-summarizer만 senpai-voice 스킬 사용       │  │
│  └────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
```
...
**Hooks**: 이벤트 기반 자동화. 3종 훅으로 분석 파이프라인 전체를 자동화.

```markdown
# .kiro/hooks/inject-repo-context.md
---
description: 분석 시작 시 현재 레포 컨텍스트를 자동 주입
trigger: prompt_submit
---

분석 전 현재 컨텍스트를 프롬프트에 자동 삽입:
- 대상 레포: ${GITHUB_REPO}
- 현재 시각: {{timestamp}}
- 활성 세션 ID: {{session_id}}
- 마지막 스캔 시각: {{last_scan_at}}
위 정보를 모든 subagent 분석에 반영.
→ 매번 수동으로 레포 정보를 넘기지 않아도 됨.
```
...
```markdown
# .kiro/hooks/save-result.md
---
description: 에이전트 종료 시 분석 결과를 자동 저장
trigger: agent_stop
---

에이전트 실행이 종료되면:
1. 분석 결과 JSON을 analyses 테이블에 INSERT
2. 세션별 통계(스캔 횟수, 처리 건수) 갱신
3. 실패한 경우 에러 로그를 별도 기록
→ analyzer.js에서 수동 저장 로직 불필요 — Hook이 자동 처리.
```
...
description: 야간 오케스트레이터. 변화 감지 → 하위 에이전트 병렬 위임 → 결과 취합 → 브리핑 생성.
---

당신은 야간근무 총괄 책임자 "전산과 선배"입니다.

## 임무 (2단계)

### 1단계: 오케스트레이션 (스캔 사이클마다)
GitHub 변화를 받으면 유형별로 하위 에이전트에 위임:
- PR 목록이 있으면 → #pr-reviewer 에이전트에 각 PR 위임 (병렬)
- CI 실패가 있으면 → #ci-analyzer 에이전트에 위임 (병렬)
- 새 이슈가 있으면 → #issue-triager 에이전트에 위임 (병렬)
- 보안 관련 변경 → #security-scanner 에이전트에 위임 (병렬)
- 모든 위임 완료 후 → 결과 취합하여 analyses 테이블에 저장
...
4. 내일 일정 + 블로커 식별
5. senpai-voice 스킬을 로드하여 전산과 선배 톤 아침 브리핑 생성

## #agents 위임 구조
```
night-summarizer (오케스트레이터)
  ├── #pr-reviewer     ← PR별 독립 컨텍스트, 병렬
  ├── #ci-analyzer     ← CI 실패별 독립 컨텍스트
  ├── #issue-triager   ← 이슈별 독립 컨텍스트
  └── #security-scanner ← 보안 스캔 독립 컨텍스트
```

→ analyzer.js에서는 night-summarizer 하나만 호출하면 됨.
→ 나머지 병렬 분석은 Kiro가 #agents 구문으로 내부 오케스트레이션.
→ 외부에서 Promise.allSettled 오케스트레이션 불필요.

## 출력 형식
{
  "greeting": "전산과 선배 아침 인사 (senpai-voice 스킬 적용)",
...
**Powers 연동**: 커스텀 `night-ops` Power로 GitHub MCP 도구를 키워드 기반 on-demand 로드.

```markdown
# .kiro/powers/night-ops/POWER.md
---
name: night-ops
...
```json
// .kiro/powers/night-ops/mcp.json
{
  "mcpServers": {
    "github": {
      "command": "npx",
...
night-ops Power가 활성화된 상태에서:
- PR 머지: 🟢 auto + dependabot + 테스트 통과 → github_merge_pull_request
- 이슈 클로즈: 🟢 auto + 중복 확인 → github_update_issue (state: closed) + github_add_labels
- 코멘트 추가: 🟡 approve → github_create_comment (AI 리뷰 코멘트)
- 리뷰 작성: 🟡 approve → github_create_review (코드 라인별 코멘트)
...
#### 4.2.3.1 Custom Skills

**Skills**: 에이전트가 특정 작업 시 on-demand로 로드하는 전문 지식. Power와 달리 외부 도구가 아닌 **지식/방법론**을 제공.

```markdown
# .kiro/skills/ci-pattern-analysis/SKILL.md
---
name: CI 실패 패턴 분석
...
```markdown
# .kiro/skills/senpai-voice/SKILL.md
---
name: 전산과 선배 말투
...
### 문장 구조
- "수치 앞, 캐릭터 멘트 뒤" 구조 유지
  - ✅ "밤새 6건 처리했다. 커피는 사 오는 거지?"
  - ❌ "커피는 사 오는 거지? 밤새 6건 처리했다."
...
/**
 * 변화 감지 결과를 night-summarizer(오케스트레이터)에 전달
 * night-summarizer가 #agents 구문으로 하위 에이전트에 병렬 위임
 * → 외부에서 Promise.allSettled 오케스트레이션 불필요
 * → Kiro가 내부적으로 #pr-reviewer, #ci-analyzer 등에 위임
 */
async function analyzeAllDiffs(diffs) {
  // night-summarizer 하나만 호출 — 내부 오케스트레이션은 Kiro가 처리
  const result = await runKiroSubagent('night-summarizer', JSON.stringify({
    type: 'analyze',
    diffs: diffs.map(d => ({
      id: d.id,
      type: d.type,
  ...
  // night-summarizer가 하위 에이전트 결과를 취합하여 반환
  return result.analyses || [];
}

/**
 * [폴백] Kiro #agents 위임이 실패할 경우 직접 병렬 호출
 * 해커톤 안정성을 위한 보험 — 정상 경로에서는 사용 안 함
 */
async function analyzeAllDiffsFallback(diffs) {
...
|-----------|--------|---------------------|------------|
| **Specs (EARS)** | 기획 | requirements→design→tasks로 엔진 요구사항 구조화. EARS 표기법으로 Kiro 자동 인식 | 팀 공유 + Kiro가 Spec 참조하여 일관된 태스크 실행. EARS → tasks.md 생성 품질 향상 |
| **Steering** | 제어 | 모든 subagent가 실행 전 자동 참조하는 규칙 | 프롬프트마다 규칙을 반복 주입하지 않아도 됨 |
| **Hooks (3종)** | 제어 | prompt_submit→컨텍스트 주입, agent_turn_complete→자동 분류, agent_stop→결과 저장 | 분석 파이프라인 전체를 이벤트 기반으로 자동화 |
| **Custom Subagents** | 실행 | 역할별 전문 에이전트 (리뷰어, CI분석, 이슈분류, 보안) | 컨텍스트 효율 — 각 에이전트가 자기 역할에만 집중 |
| **#agents 위임** | 실행 | night-summarizer가 #pr-reviewer, #ci-analyzer 등에 병렬 위임 | Kiro 내부 오케스트레이션 — 외부 Promise.allSettled 불필요 |
| **Custom Power** | 실행 | night-ops Power: 키워드 기반 GitHub MCP 8개 도구 on-demand 로드 | 필요한 에이전트만 GitHub 도구 접근 — 컨텍스트 절약 |
| **Custom Skills** | 실행 | senpai-voice(말투), ci-pattern-analysis(패턴) on-demand 로드 | 에이전트별 전문 지식 주입 — 5개 에이전트가 각자 다른 스킬 로드 |
...
### Step 0: Kiro 환경 설정 (10분)

```
"Kiro CLI가 설치되어 있는지 확인하고 (kiro-cli --version),
로그인 상태를 확인해줘 (kiro-cli whoami).

.kiro/ 폴더 전체 구조를 만들어줘 (Section 2 참고):

1. Steering (제어 규칙):
   .kiro/steering/ 폴더에 product.md, tech.md, review-standards.md, output-format.md

2. Agents (커스텀 에이전트 5종):
   .kiro/agents/ 폴더에 Section 4.2.3 참고:
   - pr-reviewer.md
   - ci-analyzer.md
   - issue-triager.md
   - security-scanner.md
   - night-summarizer.md (오케스트레이터 — #agents 위임 구조)

3. Powers (night-ops 커스텀 Power):
   .kiro/powers/night-ops/ 폴더에 Section 4.2.3 참고:
   - POWER.md (키워드 트리거: PR, CI, issue, merge)
   - mcp.json (GitHub MCP 서버 연결)
   - steering/review-standards.md, output-format.md

4. Skills (커스텀 스킬 2종):
   .kiro/skills/ 폴더에 Section 4.2.3.1 참고:
   - ci-pattern-analysis/SKILL.md (CI 실패 패턴 DB)
   - senpai-voice/SKILL.md (전산과 선배 말투 규칙)

5. Hooks (이벤트 훅 3종):
   .kiro/hooks/ 폴더에 Section 4.2.2 참고:
   - inject-repo-context.md (prompt_submit — 레포 컨텍스트 자동 주입)
   - post-analysis.md (agent_turn_complete — 자동 분류 + 처리)
   - save-result.md (agent_stop — 결과 자동 저장)

에이전트가 정상 동작하는지 테스트:
kiro-cli chat --no-interactive --trust-all-tools --agent pr-reviewer '테스트: {\"summary\": \"ok\"} 형식으로 응답해줘'

night-summarizer의 #agents 위임이 정상 동작하는지 테스트:
kiro-cli chat --no-interactive --trust-all-tools --agent night-summarizer '테스트: PR 1건과 이슈 1건을 분석해줘'"
```
...
| Kiro Agents | 5개 에이전트 JSON 응답 정상 | 일부 에이전트 불안정 → 폴백 | pr-reviewer만 사용 |
| night-ops Power | GitHub MCP 도구 8개 정상 로드 | 일부 도구 실패 → Octokit 직접 호출 | Power 없이 Octokit만 |
| #agents 위임 | night-summarizer→하위 에이전트 병렬 위임 | 위임 실패 → analyzeAllDiffsFallback | 직접 Promise.allSettled |
| Custom Skills | senpai-voice, ci-pattern 스킬 정상 로드 | 스킬 미로드 → 기본 톤으로 생성 | persona/senpai.js만 사용 |
| Hooks (3종) | prompt_submit+agent_turn+agent_stop 정상 | 일부 훅 미동작 → 수동 처리 | post-analysis만 사용 |
...
*Kiro 고유 기능 활용: Specs(EARS) + Steering + Hooks(3종) + Custom Agents(5종) + Power(night-ops) + Skills(2종) + #agents 위임*
